...
-- Handles tables with different user ID column names
-- ============================================================================

-- ============================================================================
-- SECTION 1: ENABLE RLS (Skip if already enabled)
-- ============================================================================
...
-- ============================================================================
-- SECTION 2: CREATE RLS POLICIES (Only for tables with user_id column)
-- ============================================================================
...
CREATE POLICY "Users can create business profile" ON public.business_profiles FOR INSERT WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Users can view own campaigns" ON public.email_campaigns FOR SELECT USING (auth.uid() = user_id);
...
CREATE POLICY "Users can update own campaigns" ON public.email_campaigns FOR UPDATE USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can delete own campaigns" ON public.email_campaigns;
CREATE POLICY "Users can delete own campaigns" ON public.email_campaigns FOR DELETE USING (auth.uid() = user_id);

-- Email engagement
DROP POLICY IF EXISTS "Users can view own engagement" ON public.email_engagement;
CREATE POLICY "Users can view own engagement" ON public.email_engagement FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Service role can insert engagement" ON public.email_engagement;
CREATE POLICY "Service role can insert engagement" ON public.email_engagement FOR INSERT WITH CHECK (auth.role() = 'service_role');

-- Email subscribers
DROP POLICY IF EXISTS "Users can view own subscribers" ON public.email_subscribers;
CREATE POLICY "Users can view own subscribers" ON public.email_subscribers FOR SELECT USING (auth.uid() = user_id);
...
-- Campaigns
DROP POLICY IF EXISTS "Users can view own campaigns (campaigns table)" ON public.campaigns;
CREATE POLICY "Users can view own campaigns (campaigns table)" ON public.campaigns FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can create campaigns (campaigns table)" ON public.campaigns;
CREATE POLICY "Users can create campaigns (campaigns table)" ON public.campaigns FOR INSERT WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can update own campaigns (campaigns table)" ON public.campaigns;
CREATE POLICY "Users can update own campaigns (campaigns table)" ON public.campaigns FOR UPDATE USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can delete own campaigns (campaigns table)" ON public.campaigns;
CREATE POLICY "Users can delete own campaigns (campaigns table)" ON public.campaigns FOR DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "Service role can insert usage" ON public.usage_records FOR INSERT WITH CHECK (auth.role() = 'service_role');
...
CREATE POLICY "Service role can manage subscriptions" ON public.user_subscriptions FOR ALL WITH CHECK (auth.role() = 'service_role');
...
CREATE POLICY "Service role can manage billing" ON public.billing_events FOR ALL WITH CHECK (auth.role() = 'service_role');
...
CREATE POLICY "Users can view own scheduled content" ON public.scheduled_content FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can manage own scheduled content" ON public.scheduled_content;
CREATE POLICY "Users can manage own scheduled content" ON public.scheduled_content FOR ALL USING (auth.uid() = user_id);

-- Email variants
DROP POLICY IF EXISTS "Users can view own email variants" ON public.email_variants;
CREATE POLICY "Users can view own email variants" ON public.email_variants FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can manage own email variants" ON public.email_variants;
CREATE POLICY "Users can manage own email variants" ON public.email_variants FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY "Users can view own post performance" ON public.post_performance FOR SELECT USING (auth.uid() = user_id);

-- Post analytics
DROP POLICY IF EXISTS "Users can view own post analytics" ON public.post_analytics;
CREATE POLICY "Users can view own post analytics" ON public.post_analytics FOR SELECT USING (auth.uid() = user_id);
...
CREATE POLICY "Users can view own marketing reports" ON public.marketing_reports FOR SELECT USING (auth.uid() = user_id);

-- Strategy insights
DROP POLICY IF EXISTS "Users can view own strategy insights" ON public.strategy_insights;
CREATE POLICY "Users can view own strategy insights" ON public.strategy_insights FOR SELECT USING (auth.uid() = user_id);

-- Competitor activity
DROP POLICY IF EXISTS "Users can view own competitor activity" ON public.competitor_activity;
CREATE POLICY "Users can view own competitor activity" ON public.competitor_activity FOR SELECT USING (auth.uid() = user_id);

-- Brand mentions
DROP POLICY IF EXISTS "Users can view own brand mentions" ON public.brand_mentions;
CREATE POLICY "Users can view own brand mentions" ON public.brand_mentions FOR SELECT USING (auth.uid() = user_id);

-- Brand intelligence
DROP POLICY IF EXISTS "Users can view own brand intelligence" ON public.brand_intelligence;
CREATE POLICY "Users can view own brand intelligence" ON public.brand_intelligence FOR SELECT USING (auth.uid() = user_id);

-- Trending hashtags
DROP POLICY IF EXISTS "Users can view own trending hashtags" ON public.trending_hashtags;
CREATE POLICY "Users can view own trending hashtags" ON public.trending_hashtags FOR SELECT USING (auth.uid() = user_id);

-- Content performance
DROP POLICY IF EXISTS "Users can view own content performance" ON public.content_performance;
CREATE POLICY "Users can view own content performance" ON public.content_performance FOR SELECT USING (auth.uid() = user_id);
...
CREATE POLICY "Users can manage own ab tests" ON public.ab_tests FOR ALL USING (auth.uid() = user_id);

-- A/B test variants
DROP POLICY IF EXISTS "Users can view own ab test variants" ON public.ab_test_variants;
CREATE POLICY "Users can view own ab test variants" ON public.ab_test_variants FOR SELECT USING (auth.uid() = user_id);

-- AI learning data
DROP POLICY IF EXISTS "Users can view own ai learning data" ON public.ai_learning_data;
CREATE POLICY "Users can view own ai learning data" ON public.ai_learning_data FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Service role can insert ai learning data" ON public.ai_learning_data;
CREATE POLICY "Service role can insert ai learning data" ON public.ai_learning_data FOR INSERT WITH CHECK (auth.role() = 'service_role');

-- AI optimization rules
DROP POLICY IF EXISTS "Users can view own ai optimization" ON public.ai_optimization_rules;
CREATE POLICY "Users can view own ai optimization" ON public.ai_optimization_rules FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Service role can manage ai optimization" ON public.ai_optimization_rules;
CREATE POLICY "Service role can manage ai optimization" ON public.ai_optimization_rules FOR ALL WITH CHECK (auth.role() = 'service_role');

-- Autopilot settings
DROP POLICY IF EXISTS "Users can view own autopilot settings" ON public.autopilot_settings;
CREATE POLICY "Users can view own autopilot settings" ON public.autopilot_settings FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can manage own autopilot settings" ON public.autopilot_settings;
CREATE POLICY "Users can manage own autopilot settings" ON public.autopilot_settings FOR ALL USING (auth.uid() = user_id);

-- Generated media
DROP POLICY IF EXISTS "Users can view own generated media" ON public.generated_media;
CREATE POLICY "Users can view own generated media" ON public.generated_media FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can insert own generated media" ON public.generated_media;
CREATE POLICY "Users can insert own generated media" ON public.generated_media FOR INSERT WITH CHECK (auth.uid() = user_id);
...
-- ============================================================================
-- SECTION 3: ADD PERFORMANCE INDEXES (Only on tables that exist)
-- ============================================================================
