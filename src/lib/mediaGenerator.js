...
Return ONLY the prompt, no explanations.`;
    ...
    // 1. Try OpenAI DALL-E first (you have API key configured)
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.data && data.data[0] && data.data[0].url) {
            console.log('✅ Image generated with OpenAI DALL-E 3');
            return {
              success: true,
              imageUrl: data.data[0].url,
              prompt: prompt,
              enhancedPrompt: enhancedPrompt,
              note: 'Image generated by OpenAI DALL-E 3',
        ...
        errors.push('OpenAI DALL-E: ' + openaiError.message);
        console.log('OpenAI DALL-E failed, trying next...');
      }
    }
    
    // 2. Try Pollinations AI (Completely FREE, good quality, no API key)
    try {
    ...
    if (deepaiKey) {
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          if (data.output_url) {
            console.log('✅ Image generated with DeepAI');
            return {
              success: true,
    ...
    if (leonardoKey) {
      try {
        ...
        if (response.ok) {
          ...
          if (imageUrl) {
            console.log('✅ Image generated with Leonardo.ai');
            return {
              success: true,
              imageUrl: imageUrl,
              prompt: prompt,
    ...
    try {
      ...
      if (hfResponse.ok) {
        ...
        console.log('✅ Image generated with Hugging Face');
        return {
          success: true,
          imageUrl: imageUrl,
          prompt: prompt,
...
export const generateInfographicContent = async (topic, businessContext = '') => {
  try {
    console.log('📊 Generating infographic content...');
...
export const repurposeContent = async (originalContent, targetFormats, businessContext = '') => {
  try {
    console.log('♻️ Repurposing content...');
